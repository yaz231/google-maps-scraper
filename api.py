...
async def scrape(req: ScrapeRequest):
    ...
    logger.info("city: %s", req.city)
    logger.info("latitude: %s, longitude: %s", req.latitude, req.longitude)
    ...
    logger.info("job_payload: %s", json.dumps(job_payload))

    async with httpx.AsyncClient(timeout=30.0) as client:
        # Create the job
        job_url = f"{SCRAPER_ADDR}/api/v1/jobs"
        logger.info("Calling POST %s", job_url)
        try:
            resp = await client.post(job_url, json=job_payload)
        ...
        logger.info("Scraper response: status_code=%s, body=%s", resp.status_code, resp.text)

        if resp.status_code != 201:
            logger.error("Scraper rejected job: status_code=%s, body=%s", resp.status_code, resp.text)
            raise HTTPException(
                status_code=502,
        ...
        while elapsed < POLL_TIMEOUT:
            ...
            logger.info("Polling attempt %d (elapsed %ds): job_id=%s", poll_count, elapsed, job_id)
            ...
            if status_resp.status_code != 200:
                logger.info("Job %s poll: non-200 status_code=%s, body=%s", job_id, status_resp.status_code, status_resp.text)
                continue

            job_data = status_resp.json()
            status = job_data.get("Status", job_data.get("status", ""))
            logger.info("Job %s status response: %s", job_id, json.dumps(job_data))

            if status == "ok":
                logger.info("Job completed! Downloading results...")
                break
            elif status == "failed":
                logger.error("Job %s FAILED. Full response: %s", job_id, json.dumps(job_data))
                raise HTTPException(
                    status_code=500,
        ...
        logger.info("Download response: status_code=%s, content_length=%d", dl_resp.status_code, len(dl_resp.text))

        if dl_resp.status_code != 200:
            logger.error("Failed to download results for job %s: status_code=%s, body=%s", job_id, dl_resp.status_code, dl_resp.text)
            raise HTTPException(
                status_code=500,
                detail=f"Failed to download results: {dl_resp.text}",
            )

        # Log CSV row count before parsing
        csv_rows = dl_resp.text.strip().split("\n")
        logger.info("CSV parsing: found %d rows", len(csv_rows))

        shops = parse_csv_to_shops(dl_resp.text)
        logger.info("Extracted %d valid shops", len(shops))

        # Cap at 50
        shops = shops[:50]

        logger.info("Returning success response: count=%d", len(shops))
